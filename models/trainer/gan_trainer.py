...
class GANTrainer():
    ...
    def __init__(self,
        ...
        print("%d images detected" % int(len(self.getDataset(0, size=8))))
    ...
    def trainOnEpoch(self,
        ...
        for item, data in enumerate(tqdm(dbLoader), 0):
            ...
            if i % self.lossIterEvaluation == 0:
                ...
                tqdm.write('[%d : %6d] loss G : %.3f loss D : %.3f' % (scale, i,
                      self.lossProfile[-1]["lossG"][-1],
                      self.lossProfile[-1]["lossD"][-1]))
                G_Loss_sum+=self.lossProfile[-1]["lossG"][-1]
                D_Loss_sum+=self.lossProfile[-1]["lossD"][-1]
                sum_count+=1
        ...
        return True, G_Loss_sum/sum_count, D_Loss_sum/sum_count
