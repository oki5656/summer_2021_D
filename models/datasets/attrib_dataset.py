...
class AttribDataset(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None,):
        self.totAttribSize = 0
        self.hasAttrib = False
        self.pathdb = dataset_dir
        self.transform = transform
        self.shiftAttrib = None
        self.stats = None
        self.pathMask = None
        ...
        if bndbox_file_path is None:
          self.img_labels = [[img_name, self.annotation_dict[img_name[:9]][0]] for img_name in os.listdir(self.img_dir)] # [[string: img_name, int: label], ...]
        else:
          with open(bndbox_file_path, 'r') as f:
            img_name_bndbox_list = f.readlines()
          self.img_labels = [[img_name_bndbox.split()[0], self.annotation_dict[img_name_bndbox.split()[0][:9]][0], 
                              [int(n) for n in img_name_bndbox.split()[1:5]]]
                              for img_name_bndbox in img_name_bndbox_list] # [[string: img_name, int: label, [int xmin, int ymin, int xmax, int ymax]], ...]
    ...
    def __getitem__(self, idx):
        #print("#### PATH ####",self.img_dir, self.img_labels[idx][0])
        ...
        if 2 < len(self.img_labels[idx]): 
          xmin, ymin, xmax, ymax = self.img_labels[idx][2]
          w = xmax - xmin
          h = ymax - ymin
          if h > w: xmin += (w-h)//2; xmax = xmin + h
          else: ymin += (h-w)//2; ymax = ymin + w
          img = img.crop([xmin, ymin, xmax, ymax]) #正方形にcrop

          img_np = np.array(img)
          # 画像範囲外をmirror
          if xmin < 0: img_np[:, 0:-xmin+1] = np.fliplr(img_np[:, -xmin:-xmin*2+1])
          if ymin < 0: img_np[0:-ymin+1, :] = np.flipud(img_np[-ymin:-ymin*2+1, :])
          if xmax > img_w: img_np[:, img_w-xmax-1:] = np.fliplr(img_np[:, (img_w-xmax)*2-1:img_w-xmax])
          if ymax > img_h: img_np[img_h-ymax-1:, :] = np.flipud(img_np[(img_h-ymax)*2-1:img_h-ymax, :])
